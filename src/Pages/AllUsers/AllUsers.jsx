...
const AllUsers = () => {
    const { data: users = [], refetch } = useQuery(['users'], async () => {
        const res = await fetch('https://final-project-server-side-ataullahmesbah.vercel.app/users')
        return res.json();
    })

    const handleMakeAdmin = user => {
        fetch(`https://final-project-server-side-ataullahmesbah.vercel.app/users/admin/${user._id}`, {
            method: 'PATCH'
        })
            .then(res => res.json())
            .then(data => {
    ...
    const handleMakeInstructor = user => {
        fetch(`https://final-project-server-side-ataullahmesbah.vercel.app/users/instructor/${user._id}`, {
          method: 'PATCH'
        })
          .then(res => res.json())
          .then(data => {
        ...
        Swal.fire({
            ...
            .then((result) => {
                if (result.isConfirmed) {
                    fetch(`https://final-project-server-side-ataullahmesbah.vercel.app/users/${user._id}`, {
                        method: 'DELETE'
                    })
                        .then(res => res.json())
                        .then(data => {
